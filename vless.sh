...
vr_il_family_guard_state() {
  ...
  # Readers use the same lock as rebuild/delete operations, so an audit cannot
  # observe the short interval between deleting and recreating the family rule.
  vr_il_lock
  rules="$(
    nft -a list chain inet "$VR_IL_TABLE" "$VR_IL_INPUT_CHAIN" 2>/dev/null || true
  )"
...
vr_pq_snapshot() {
  ...
  # Use the writer lock for reads as well.  A normal save/rebuild transaction
  # sets REBUILD_PENDING=1 only while holding this lock; waiting here prevents
  # audits from displaying a harmless intermediate state as `stale`.
  vr_pq_lock
  meta="$(vr_quota_meta_file "$port")"
  if [[ ! -f "$meta" ]]; then
    printf 'none|0|0|0\n'
