...
vr_il_state() {
  local port="$1" meta ip_version
  meta="$(vr_iplimit_meta_file "$port")"
  [[ -f "$meta" ]] || { printf 'none\n'; return 0; }
  ip_version="$(vr_meta_get "$meta" IP_VERSION 2>/dev/null || true)"
  ip_version="${ip_version:-4}"
  if nft list set inet "$VR_IL_TABLE" "$(vr_il_set_name "$port" "$ip_version")" >/dev/null 2>&1 \
    && vr_il_rule_comment_exists "$(vr_il_comment_refresh "$port")" \
    && vr_il_rule_comment_exists "$(vr_il_comment_claim "$port")" \
    && vr_il_rule_comment_exists "$(vr_il_comment_drop "$port")" \
    && vr_il_rule_comment_exists "$(vr_il_comment_family "$port")"
  then
    printf 'active\n'
  else
    printf 'stale\n'
...
TEMP_META=""
IP_VERSION=""
for candidate in "$VR_TEMP_STATE_DIR"/*.env; do
  [[ -f "$candidate" ]] || continue
  if [[ "$(vr_meta_get "$candidate" PORT 2>/dev/null || true)" == "$PORT" ]]; then
    TEMP_META="$candidate"
    IP_VERSION="$(vr_meta_get "$candidate" IP_VERSION 2>/dev/null || true)"
    break
  fi
done
