...
vr_il_family_guard_state_unlocked() {
  local port="$1" rules comment
  rules="$(
    nft -a list chain inet "$VR_IL_TABLE" "$VR_IL_INPUT_CHAIN" 2>/dev/null || true
  )"
  comment="$(vr_il_comment_family "$port")"
...
vr_il_family_guard_state() {
  ...
  result="$(vr_il_family_guard_state_unlocked "$1")"
  (( acquired == 0 )) || vr_il_unlock
  printf '%s\n' "$result"
}
...
vr_pq_counter_bytes() {
  ...
  output="$(nft -n list counter inet "$VR_PQ_TABLE" "$obj" 2>/dev/null)" || return 1
  value="$(
    awk '/bytes/ {
      for (i = 1; i <= NF; i++) {
        if ($i == "bytes") {
...
vr_pq_quota_used_bytes() {
  local obj="$1" output value
  output="$(nft -n list quota inet "$VR_PQ_TABLE" "$obj" 2>/dev/null)" || return 1
  value="$(
    awk '/used/ {
      for (i = 1; i <= NF; i++) {
        if ($i == "used") {
...
vr_pq_live_used_bytes() {
  ...
  # Read both independent views.  Never turn a read/parse failure into zero:
  # pq-save would otherwise persist the false zero and rebuild a fresh quota,
  # making the displayed usage jump backwards.
  if quota_b="$(vr_pq_quota_used_bytes "$(vr_pq_quota_obj "$port")")"; then
    quota_ok=1
  fi
  if in_b="$(vr_pq_counter_bytes "$(vr_pq_counter_in "$port")")" \
    && out_b="$(vr_pq_counter_bytes "$(vr_pq_counter_out "$port")")"
  then
  ...
  if (( quota_ok == 1 && counter_ok == 1 )); then
    # The quota object normally reaches the exact limit, while the counters
    # behind the drop rule can be short by the threshold-crossing packet.
    # max() also prevents a transiently low quota reading from moving usage
    # backwards when the counters still hold the larger monotonic value.
  ...
  # A fully exhausted saved quota intentionally has no quota/counter objects;
  # only the two unconditional drop rules remain.  In that one valid state,
  # live usage is zero because the full amount is already in SAVED_USED_BYTES.
  meta="$(vr_quota_meta_file "$port")"
  remaining="$(vr_meta_get "$meta" LIMIT_BYTES 2>/dev/null || true)"
  if [[ "$remaining" == "0" ]] \
...
vr_pq_snapshot_unlocked() {
  ...
  original="$(vr_meta_get "$meta" ORIGINAL_LIMIT_BYTES 2>/dev/null || true)"
  saved="$(vr_meta_get "$meta" SAVED_USED_BYTES 2>/dev/null || true)"
  ...
  # If PENDING is still set after acquiring the writer lock, the previous
  # transaction was interrupted.  This is a real stale state, not a live race.
  if [[ "$rebuild_pending" == "1" ]]; then
    printf 'stale|%s|%s|%s\n' "$original" "$used" "$left"
    return 0
  fi

  if ! live="$(vr_pq_live_used_bytes "$port")"; then
    printf 'stale|%s|%s|%s\n' "$original" "$used" "$left"
    return 0
  fi
  used=$((saved + live))
  ...
  # One chain snapshot per direction keeps state and percentage from being
  # calculated from different generations of the nftables ruleset.
  input_rules="$(
    nft -a list chain inet "$VR_PQ_TABLE" "$VR_PQ_INPUT_CHAIN" 2>/dev/null || true
  )"
  output_rules="$(
    nft -a list chain inet "$VR_PQ_TABLE" "$VR_PQ_OUTPUT_CHAIN" 2>/dev/null || true
  )"

  if (( left <= 0 )); then
    if [[ "$input_rules" == *"comment \"$(vr_pq_comment_drop_in "$port")\""* ]] \
      && [[ "$output_rules" == *"comment \"$(vr_pq_comment_drop_out "$port")\""* ]]
    then
      state='exhausted'
    else
      state='stale'
    fi
  elif nft list counter inet "$VR_PQ_TABLE" "$(vr_pq_counter_in "$port")" >/dev/null 2>&1 \
    ...
    && [[ "$input_rules" == *"comment \"$(vr_pq_comment_drop_in "$port")\""* ]] \
    && [[ "$input_rules" == *"comment \"$(vr_pq_comment_count_in "$port")\""* ]] \
    && [[ "$output_rules" == *"comment \"$(vr_pq_comment_drop_out "$port")\""* ]] \
    && [[ "$output_rules" == *"comment \"$(vr_pq_comment_count_out "$port")\""* ]]
  then
    state='active'
  else
    state='stale'
...
vr_pq_snapshot() {
  ...
  result="$(vr_pq_snapshot_unlocked "$1")"
  (( acquired == 0 )) || vr_pq_unlock
  printf '%s\n' "$result"
}

vr_pq_state() {
  local snapshot state
  snapshot="$(vr_pq_snapshot "$1")"
...
vr_pq_save_one() {
  ...
  # Never persist/rebuild from an unreadable snapshot.  Treating a failed
  # nft read as zero can reset a nearly exhausted quota back to 0% usage.
  live="$(vr_pq_live_used_bytes "$port")" || return 1
...
vr_ensure_runtime_dirs
# Wait for any save/rebuild transaction and keep one generation of quota state
# stable for the complete audit.
vr_pq_lock
...
quota_summary() {
  local port="$1"
  local meta snapshot state original used left
  meta="$(vr_quota_meta_file "$port")"
  if [[ ! -f "$meta" ]]; then
    printf 'none|-|-|-|-\n'
    return 0
...
vr_ensure_runtime_dirs
# Read temporary-node metadata, quota state and IP-limit rules under the same
# lock order used by writers.  This removes harmless audit races while keeping
# genuine incomplete transactions visible as `stale`.
...
for TAG in $(vr_collect_temp_tags); do
  ...
  if [[ "$LANDING" == "nat" ]]; then
    ...
    if [[ -n "$AUDIT_WG_IF" && "$AUDIT_MARK" =~ ^[0-9]+$ ]]; then
      ...
      AUDIT_ROUTE="$(ip route get 1.1.1.1 mark "$AUDIT_MARK" 2>/dev/null || true)"
      grep -qE "\bdev ${AUDIT_WG_IF}\b" <<<"$AUDIT_ROUTE" || AUDIT_RC=1
    fi
  fi
...
UNIT
  ...
  # The add helpers keep their writer locks for process-level batch operations.
  # Release them before systemd starts vless_run_temp.sh, whose startup gate
  # takes short read locks.  Otherwise parent and service wait on each other.
  vr_il_unlock
  vr_pq_unlock
  systemctl daemon-reload
...
export VR_TEMP_LOCK_HELD=1
# Quota/IP locks are scoped to individual read or repair operations.  Do not
# hold them while stopping or restarting a service: the service startup/stop
# gates use the same locks and a process-wide hold would deadlock systemd.
rc=0
now="$(date +%s)"
...
for meta in "$VR_TEMP_STATE_DIR"/*.env; do
  ...
  if (( node_healthy == 1 )) && [[ "$landing" == "nat" ]]; then
    ...
    if (( node_healthy == 1 )); then
      ...
      grep -qE "\bdev ${wg_if}\b" <<<"$route_result" || node_healthy=0
