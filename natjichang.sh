...
if ! grep -Eq '^[[:space:]]*vr_pq_unlock[[:space:]]*\(\)' /usr/local/lib/vless-reality/quota-lib.sh \
  || ! grep -Eq '^[[:space:]]*vr_il_unlock[[:space:]]*\(\)' /usr/local/lib/vless-reality/iplimit-lib.sh
then
  echo "❌ 检测到旧版 VLESS 锁库；请先重新运行最新版 vless.sh，再安装 natjichang.sh" >&2
  exit 1
fi
...
UNIT
  ...
  # Release quota/IP writer locks before systemd runs the shared startup gate.
  # The service takes short read locks to verify fail-closed protection.
  vr_il_unlock
  vr_pq_unlock
