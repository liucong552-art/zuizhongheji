...
PostUp = ip -4 route replace prohibit default metric ${FAILCLOSE_METRIC} table ${TABLE_ID}
...
PostDown = ip -4 route replace prohibit default metric ${FAILCLOSE_METRIC} table ${TABLE_ID}
CFG
  chmod 600 "$out"
}
...
# Keep the fail-closed route present without probing a possibly absent table.
# `replace` is idempotent and avoids both first-run FIB warnings and
# `ip ... | grep -q` false failures under pipefail.
ip -4 route replace prohibit default metric "$FAILCLOSE_METRIC" table "$TABLE_ID"
...
# Verify one complete snapshot at a time.  Piping `ip` into `grep -q` while
# pipefail is enabled can turn a real match into a false failure when grep exits
# early and the upstream `ip` process receives SIGPIPE.
RULE_DUMP="$(ip -4 rule show 2>/dev/null || true)"
ROUTE_DUMP="$(ip -4 route show table "$TABLE_ID" 2>/dev/null || true)"
grep -qE "^${RULE_PRIORITY}:.*fwmark (0x$(printf '%x' "$MARK")|${MARK})(/0xffffffff)?([[:space:]]|$)" <<<"$RULE_DUMP"
grep -qE "^prohibit default.*metric ${FAILCLOSE_METRIC}([[:space:]]|$)" <<<"$ROUTE_DUMP"
if ip link show dev "$WG_IF" >/dev/null 2>&1; then
  grep -qE "^default dev ${WG_IF}.*metric ${WG_ROUTE_METRIC}([[:space:]]|$)" <<<"$ROUTE_DUMP"
  ROUTE_GET="$(ip -4 route get 1.1.1.1 mark "$MARK" 2>/dev/null || true)"
  grep -qE "\bdev ${WG_IF}\b" <<<"$ROUTE_GET"
fi
WG_NAT_GUARD
chmod 755 "$GUARD_TMP"
bash -n "$GUARD_TMP"
...
RULE_DUMP="$(ip -4 rule show 2>/dev/null || true)"
if grep -qE "^${RULE_PRIORITY}:.*fwmark (${MARK_HEX}|${MARK_DEC}).*lookup ${TABLE_ID}" <<<"$RULE_DUMP"; then
  echo "OK: ip rule 存在"
else
