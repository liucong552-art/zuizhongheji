...
id=tmp40000 \
IP_VERSION=4 \
PORT_START=40000 PORT_END=40000 \
IP_LIMIT=3 \
PQ_GIB=1 \
D=$((1*3600)) \
vless_mktemp.sh
...
```bash
vless_audit.sh --tag vless-temp-tmp1h
```
...
```bash
/usr/local/sbin/wg_nat_set_peer.sh \
  '这里是NAT公钥' \
  '10.66.66.1/24'
```
...
删除 NAT 临时节点的方法与普通临时节点相同：

```bash
FORCE=1 vless_cleanup_one.sh vless-temp-nat1h
```
