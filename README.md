...
虽然脚本内部要求 `D` 使用秒，但日常创建时不需要自己计算。直接使用下面的分钟、小时或天数模板，只修改命令最前面的数字即可。

> 命令开头的 `id=...` 是给脚本传递节点名称，不是让你单独执行 Linux 的 `id` 命令。
>
> 时间数量不要写成 `01`、`08` 这类带前导零的形式，直接写 `1`、`8`。

### 1. 按分钟创建

例如创建 30 分钟节点，最多 3 个来源 IP，总流量 1 GiB：

```bash
MINUTES=30; id="tmp${MINUTES}m" IP_VERSION=4 IP_LIMIT=3 PQ_GIB=1 D=$((MINUTES*60)) vless_mktemp.sh
```

只需要修改最前面的 `MINUTES=30`：
...
```bash
HOURS=2; id="tmp${HOURS}h" IP_VERSION=4 IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp.sh
```
...
例如创建 7 天节点，总流量 50 GiB：

```bash
DAYS=7; id="tmp${DAYS}d" IP_VERSION=4 IP_LIMIT=3 PQ_GIB=50 D=$((DAYS*24*60*60)) vless_mktemp.sh
```
...
下面示例固定使用端口 `40000`，有效期 1 小时：

```bash
HOURS=1; id=tmp40000 IP_VERSION=4 PORT_START=40000 PORT_END=40000 IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp.sh
```
...
并确认该域名的 AAAA 记录指向当前 VPS。下面示例创建 1 小时 IPv6 节点：

```bash
HOURS=1; id="tmp6-${HOURS}h" IP_VERSION=6 IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp.sh
```
...
```bash
DAYS=31; id="tmp${DAYS}d" IP_LIMIT=1 PQ_GIB=100 D=$((DAYS*24*60*60)) vless_mktemp.sh
```
...
与普通节点相同，日常创建时不需要自己计算秒数，只修改命令最前面的分钟、小时或天数。

### 1. 按分钟创建

下面示例创建 30 分钟 WG-NAT 节点：

```bash
MINUTES=30; id="nat${MINUTES}m" IP_VERSION=4 IP_LIMIT=3 PQ_GIB=1 D=$((MINUTES*60)) vless_mktemp_nat.sh
```

### 2. 按小时创建

下面示例创建 2 小时 WG-NAT 节点：

```bash
HOURS=2; id="nat${HOURS}h" IP_VERSION=4 IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp_nat.sh
```

### 3. 按天创建

下面示例创建 7 天 WG-NAT 节点，总流量 50 GiB：

```bash
DAYS=7; id="nat${DAYS}d" IP_VERSION=4 IP_LIMIT=3 PQ_GIB=50 D=$((DAYS*24*60*60)) vless_mktemp_nat.sh
```

### 4. 固定端口 `40000`

下面示例固定端口 `40000`，有效期 1 小时：

```bash
HOURS=1; id=nat40000 IP_VERSION=4 PORT_START=40000 PORT_END=40000 IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp_nat.sh
```

### 5. IPv6 入站、NAT IPv4 出口
...
```bash
HOURS=1; id="nat6-${HOURS}h" IP_VERSION=6 IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp_nat.sh
```
...
```bash
HOURS=1; id="tmp${HOURS}h" IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp.sh
```
...
```bash
HOURS=1; id="nat${HOURS}h" IP_LIMIT=3 PQ_GIB=1 D=$((HOURS*60*60)) vless_mktemp_nat.sh
```
